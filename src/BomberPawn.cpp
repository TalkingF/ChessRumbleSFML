...
#include "ChessBoard.h" // Include the Board header

BomberPawn::BomberPawn(bool colour, int row, int col) : Pawn(colour, row, col) {}

bool BomberPawn::legal_move(int new_row, int new_col, ChessBoard& board) {
    ...
    if (abs(new_col - col) == 1 && new_row == row + direction) {
        ...
        if (board.get_square(new_row, new_col).get_piece() != nullptr &&
            ...
            explode(const_cast<ChessBoard&>(board), new_row, new_col); // Added const_cast to remove const so I can edit the board (THIS WAS THE PROBLEM)
...
void BomberPawn::explode(ChessBoard& board, int center_row, int center_col) {
    // Define the bounds of the three by three area
    int start_row = std::max(center_row - 1, 0);
    int end_row = std::min(center_row + 1, ChessBoard::SIZE - 1);
    int start_col = std::max(center_col - 1, 0);
    int end_col = std::min(center_col + 1, ChessBoard::SIZE - 1);

    // Destroy any piece within the three by three area
    for (int row = start_row; row <= end_row; ++row) {
        for (int col = start_col; col <= end_col; ++col) {
            board.get_square(row, col).set_piece(nullptr);
        }
    }
}